...
void function_dilate_dap2_array(int argc, BaseType * argv[], DDS &/*dds*/, BaseType **btpp)
{
    ...
    if (btp->type() != dods_array_c)
        throw Error(malformed_expr, "function_dilate_dap2_array(): first argument must point to a Array variable.");

    Array *mask = static_cast<Array*>(btp);
    if (mask->var()->type() != dods_byte_c && mask->dimensions() == 2)
        throw Error(malformed_expr, "function_dilate_dap2_array(): first argument must point to a Two dimensional Byte Array variable.");
    ...
    if (!is_integer_type(argv[1]->type()))
        throw Error(malformed_expr, "function_dilate_dap2_array(): Expected an integer for the second argument.");
...
BaseType *function_dilate_dap4_array(D4RValueList *args, DMR &dmr)
{
    ...
    BESDEBUG("functions", "function_dilate_dap4_array() -  args->size(): " << args->size()  << endl);

    BaseType *btp = args->get_rvalue(0)->value(dmr);

    if (btp->type() != dods_array_c)
        throw Error(malformed_expr, "function_dilate_dap4_array(): first argument must point to a Array variable.");

    Array *mask = static_cast<Array*>(btp);
    if (mask->var()->type() != dods_byte_c && mask->dimensions() == 2)
        throw Error(malformed_expr, "function_dilate_dap4_array(): first argument must point to a Two dimensional Byte Array variable.");
    ...
    if (!is_integer_type(dilation_ops_count->type()))
        throw Error(malformed_expr, "function_dilate_dap4_array(): Expected an integer for the second argument.");
