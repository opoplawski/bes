...
template<typename T>
std::vector< std::vector<int> > make_normalized_mask_helper(const std::vector<libdap::Array*> dims, libdap::Array *tuples);
 ...
 */
class MakeNormalizedMaskFunction: public libdap::ServerFunction
{
public:
    MakeNormalizedMaskFunction()
    {
        setName("make_normalized_mask");
        setDescriptionString("The make_normalized_mask() function reads a number of dim_names, followed by a set of dim value tuples and builds a DAP Array object.");
        //setUsageString("make_mask(type,dim0,dim1,..dimN,dim0_value,dim1_value,...,dimN_value)");
        setRole("http://services.opendap.org/dap4/server-side-function/make_normalied_mask");
        setDocUrl("http://docs.opendap.org/index.php/Server_Side_Processing_Functions#make_normalized_mask");
        setFunction(function_dap2_make_normalized_mask);
        //setFunction(function_make_dap4_mask);
        setVersion("1.0");
    }
    virtual ~MakeNormalizedMaskFunction()
    {
    }
};
