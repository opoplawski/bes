...
void BESXMLInterface::build_data_request_plan()
{
    ...
    try {
        ...
        BESDEBUG("besxml", "====>>>>>>>>> root_name = " << root_name << " root_val = " << root_val << endl);
        ...
        while (current_node) {
            if (current_node->type == XML_ELEMENT_NODE) {
                ...
                BESDEBUG("besxml", "====>>>>>>>>> return_as = " << return_as << endl);
                BESDEBUG("besxml", "==!!!! find_transmitter = " << BESReturnManager::TheManager()->find_transmitter(return_as) << endl);
